...
### Selected adapter: CCIP-delivered VRF

The release configuration uses `SinjohCcipVrfRandomness`, specified in
[`sinjoh-ccip-vrf`](../sinjoh-ccip-vrf/SPEC.md). Randomness is generated by
Chainlink VRF v2.5 on Arbitrum One and carried to Robinhood Chain over CCIP, which
is live on `4663`.

What the raffle depends on, and nothing more:

1. `requestRandomness(roundId)` records a request and makes **no external call and
   spends no value**, so `commitRound` cannot fail on adapter funding;
2. `receiveRandomness(requestId, seed)` arrives later, from the immutable adapter
   address only, at most once per request;
3. the seed is `keccak256(requestId, word)` and is unpredictable to the attestor,
   the creator, any holder, and the raffle at commit time;
4. delivery may never arrive, which is what `randomnessTimeout` and
   `abandonRound` exist for.

One VRF word is batched across every request assigned to the same beacon, so an
hourly cadence costs one VRF request and two CCIP messages per beacon rather than
per raffle. Sharing is safe because slot indices derive from
`keccak256(..., chainid, raffle, roundId, k, seed)`: two raffles on one beacon get
uncorrelated indices. The adapter enforces the invariant that makes batching sound
...
### Alternatives, retained for other deployments

- **`SinjohEcvrfRandomness`** — a secp256k1 ECVRF proof verified on-chain against
  an immutable public key, with `alpha` bound to the committed root. It needs no
  Chainlink deployment and settles in seconds, but the key holder can compute the
  outcome before publishing and decline to publish, re-rolling across rounds at the
  cost of forfeiting them. Not used here; retained for chains with no CCIP lane, and
  only where that residual is disclosed in the interface.
- **`SinjohDirectVrfRandomness`** — a plain VRF v2.5 subscription consumer for any
  chain with a local coordinator, including Robinhood Chain if VRF is deployed
  there later.

Because the adapter is chosen per deployment behind a two-function interface,
adopting either requires no change to this specification. It does require a new
raffle: the adapter address is immutable, and an existing raffle can never be
repointed.
...
1. ~~**Randomness adapter.**~~ **Resolved:** CCIP-delivered Chainlink VRF, specified
   in [`sinjoh-ccip-vrf`](../sinjoh-ccip-vrf/SPEC.md). Blocking item before any
   mainnet raffle: confirm the Robinhood Chain ↔ Arbitrum One CCIP lane in both
   directions.
