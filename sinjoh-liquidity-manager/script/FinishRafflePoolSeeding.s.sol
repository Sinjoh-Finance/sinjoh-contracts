...
contract FinishRafflePoolSeeding {
    ...
    uint16 internal constant CARDINALITY = 1_500;

    function run() external {
        vm.startBroadcast();
        // Seed sizes measured per pool on live state (2026-08-05): the exact
        // amount that moves each tick, so nothing is wasted climbing a ladder.
        _finish(GUARD_10000, 0x05b37Fb53A299a1b874A619e1c4C404D52C36F4C, 10_000, 0.0005 ether); // RDDT
        _finish(GUARD_500, 0x2e0847E8910a9732eB3fb1bb4b70a580ADAD4FE3, 500, 0.0005 ether); // GOOGL
        _finish(GUARD_500, 0xaF3D76f1834A1d425780943C99Ea8A608f8a93f9, 500, 0.004 ether); // AAPL
        vm.stopBroadcast();
    }
    ...
    function _finish(address guard, address asset, uint24 fee, uint256 startAmount) private {
        ...
        if (card >= 2) return;
        // A prior interrupted run already wrapped ETH; spend that WETH first
        // and deposit only the shortfall.
        uint256 held = ISeedWeth(WETH).balanceOf(msg.sender);
        if (held < startAmount) ISeedWeth(WETH).deposit{ value: startAmount - held }();
