...
- **Wrong assumption:** A stable proxy address and runtime hash do not prove that tuple selectors remain callable; the active implementation must also be verified.
...
- **Immutable choice:** letscash.fun fee recipients and the selected standard/custom token supply are fixed by launch calldata and must be server-verified before publication.
