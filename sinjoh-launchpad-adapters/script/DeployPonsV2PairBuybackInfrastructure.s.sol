...
interface VmPonsV2PairBuybackInfrastructure {
    function startBroadcast() external;
    function stopBroadcast() external;
}

/// @notice Deploys the pair-capable pons v2 buyback route: one singleton swap
/// adapter serving native and custom-pair launches alike, and its signed-floor
/// price guard. Supersedes the native-only route for new launches; existing
/// routes (none, at deploy time) are untouched. Broadcast with the keystore
/// account, matching the treasury deploy convention:
///
///   forge script script/DeployPonsV2PairBuybackInfrastructure.s.sol:DeployPonsV2PairBuybackInfrastructure \
...
contract DeployPonsV2PairBuybackInfrastructure {
    ...
    VmPonsV2PairBuybackInfrastructure internal constant vm =
        VmPonsV2PairBuybackInfrastructure(address(uint160(uint256(keccak256("hevm cheat code")))));
    ...
    function run()
        external
        returns (SinjohPonsV2PairBuybackAdapter adapter, SinjohPonsV2PairBuybackPriceGuard guard)
    {
        ...
        if (
            address(adapter).code.length == 0 || address(guard).code.length == 0
                ...
                || adapter.v3SwapRouter() != V3_SWAP_ROUTER || adapter.memeHook() != MEME_HOOK
                || adapter.memeHookCodehash() != MEME_HOOK_HASH
