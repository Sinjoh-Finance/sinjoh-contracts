...
/// @notice Installs the public factory release with independent fixed-price mint inventories.
/// @dev Reuses every unchanged V1.0.5 creation-code store and replaces only the collection store.
contract DeployYieldBankPublicFactoryV106Mainnet is Script {
    uint256 private constant EXPECTED_CHAIN_ID = 4_663;
    address private constant V105_FACTORY = 0xc576771755F55bD8586a8fF36b14595d5a93196C;
    bytes32 public constant FACTORY_VERSION = keccak256("SINJOH_YIELD_BANK_PUBLIC_FACTORY_V1_0_6");
    ...
    function run() external returns (YieldBankPublicFactory factory) {
        ...
        YieldBankPublicFactory previous = YieldBankPublicFactory(V105_FACTORY);
        ...
        registry.deprecateFactory(V105_FACTORY);
        vm.stopBroadcast();
        ...
        console2.log("YieldBankCollectionCreationCodeStoreV106", stores.collection);
        console2.log("YieldBankPublicFactoryV106", address(factory));
    ...
    function _preflight(
        ...
        YieldBankProtocolRegistry registry = previous.registry();
        (,, bool registered, bool deprecated) = registry.factories(V105_FACTORY);
        if (
            block.chainid != EXPECTED_CHAIN_ID || broadcaster != registry.governance()
                ...
                || !registry.isFactoryAvailableForNewCollections(V105_FACTORY)
