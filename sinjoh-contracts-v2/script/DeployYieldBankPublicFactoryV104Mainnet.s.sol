...
/// @notice Installs the factory release whose redemption requirement sends the configured ERC-20
/// amount to the protocol burn address. All unchanged component creation-code stores are reused.
contract DeployYieldBankPublicFactoryV104Mainnet is Script {
    ...
    address private constant V103_FACTORY = 0x5Ef0056E4619a92CDeEa6139c29EDC76b19820c1;
    ...
    bytes32 public constant FACTORY_VERSION = keccak256("SINJOH_YIELD_BANK_PUBLIC_FACTORY_V1_0_4");
    ...
    function run() external returns (YieldBankPublicFactory factory) {
        ...
        _preflight(
            broadcaster, expectedNonce, expectedCollectionStore, expectedFactory, stores, hashes
        );
        ...
        YieldBankProtocolRegistry(REGISTRY).deprecateFactory(V103_FACTORY);
        vm.stopBroadcast();
        ...
        console2.log("YieldBankCollectionCreationCodeStoreV104", stores.collection);
        console2.log("YieldBankPublicFactoryV104", address(factory));
    ...
    function _preflight(
        ...
        (,, bool registered, bool deprecated) = registry.factories(V103_FACTORY);
        if (
            registry.governance() != broadcaster || !registered || deprecated
                || !registry.isFactoryAvailableForNewCollections(V103_FACTORY)
