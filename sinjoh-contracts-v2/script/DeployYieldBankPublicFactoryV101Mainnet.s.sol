...
/// @notice Deploys and registers the corrected public factory, then prevents the superseded
/// factory from accepting new collection registrations. Existing collections are unaffected.
contract DeployYieldBankPublicFactoryV101Mainnet is Script {
    ...
    bytes32 public constant FACTORY_VERSION = keccak256("SINJOH_YIELD_BANK_PUBLIC_FACTORY_V1_0_1");
    ...
    function run() external returns (YieldBankPublicFactory factory) {
        ...
        (,,, bool supersededDeprecated) = registry.factories(SUPERSEDED_FACTORY);
        if (supersededDeprecated) revert FactoryRegistrationMismatch();
        ...
        console2.log("YieldBankPublicFactoryV101", address(factory));
    ...
    function _verifyFactory(
        ...
        (,, bool oldRegistered, bool oldDeprecated) = registry.factories(SUPERSEDED_FACTORY);
        if (
            !registered || deprecated || version != FACTORY_VERSION
                || runtimeHash != address(factory).codehash || !oldRegistered || !oldDeprecated
                || !registry.isFactoryAvailableForNewCollections(address(factory))
                || registry.isFactoryAvailableForNewCollections(SUPERSEDED_FACTORY)
