...
/// @notice One immutable v2 Raffle for one Registry project. Winners never sign or claim manually.
/// @dev Clone-initialized once with Registry and subject already bound. There is no owner, upgrade,
/// rescue, arbitrary call, post-launch binding, or configuration setter.
contract ProjectRaffleV2 is IProjectModule, IProjectFundable {
    ...
    event RaffleCreated(
        ...
        bytes32 configHash,
        RaffleTypes.Settings configuration,
        address[] exclusions
    );
    event Deposited(
    ...
    function initialize(
        ...
        for (uint256 i; i < stockRewardLength; ++i) {
            if (subject_ == config.stockRewards[i].asset) revert InvalidSubject(subject_);
        }
        ...
        emit RaffleCreated(
            expectedProjectId, registry_, subject_, configHash, frozen, config.exclusions
        );
    }
    ...
    /// @dev The route limit controls transaction size only and never changes the round prize.
        ...
        returns (uint256 paid)
    {
        ...
        try this.payWinner(holder, net) {
            ...
            emit PaymentDeferred(
                roundId, slot, holder, share, recipientTax, recycleTax, net, bytes("")
            );
        }
    }
    ...
    function feeRemainder() external view returns (uint16) {
        return _feeRemainder;
    }

    function roundStatus(uint64 roundId) external view returns (RaffleTypes.Round memory) {
        ...
        view
    {
        ...
        if (
            config.creator == BURN_ADDRESS || config.attestor == BURN_ADDRESS
                || config.randomness == BURN_ADDRESS || config.protocolFeeRecipient == BURN_ADDRESS
                || (config.recipientTaxBps != 0 && config.taxRecipient == BURN_ADDRESS)
        ) revert InvalidAddress();
