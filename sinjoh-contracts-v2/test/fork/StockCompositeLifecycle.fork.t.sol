...
import { StockCompositeInfrastructureForkTest } from "./StockCompositeInfrastructure.fork.t.sol";
import {
...
contract StockCompositeLifecycleForkTest is StockCompositeInfrastructureForkTest {
    ...
    function _checkLPMaintenance() private {
        ...
        StockCompositeLPAdapter.LPDeposit memory deployment = abi.decode(_lpEntry(idle), (StockCompositeLPAdapter.LPDeposit));
        vm.prank(address(0xBAD)); vm.expectRevert(StockCompositeLPAdapter.Unauthorized.selector);
        facade.rebalanceLP(0,100,"",deployment.assetsToDeploy,deployment.minimumPositionUnits,deployment.adapterData);
