...
interface IStockForkPool {
    function initialize(uint160) external;
}

interface IStockForkWETH {
    function deposit() external payable;
}

interface IStockForkManager {
...
contract StockCompositeInfrastructureForkTest is Test {
    ...
    function _deployInfrastructure() internal {
        ...
        IStockForkFactory(factory).enableFeeAmount(100, 1);
        registrationPool = IStockForkFactory(factory).createPool(WETH, USDG, 100);
        (uint160 sqrtPrice, int24 tick,,,,,) = IYieldBankV3Pool(USDG_POOL).slot0();
        IStockForkPool(registrationPool).initialize(sqrtPrice);
        // Explicit fork-only infrastructure bootstrap; funds never enter an NFT treasury.
        vm.deal(address(this), 0.01 ether);
        IStockForkWETH(WETH).deposit{ value: 0.01 ether }();
        ...
        assertEq(IStockForkManager(manager).ownerOf(ids[0]), address(this));
        ...
        IStockForkFactory(factory).setOwner(COLLECTION.collectionTimelock());
    }
