...
| Router | `ProjectRouterV2` typed route versions and exact retry escrow | unit, fuzz, cross-module split/recovery and system conservation suites | presets own adapters/routes; superseded escrow is recovery-only; one-call work status |
...
| Airdrop | `ProjectAirdropV2` + deterministic TypeScript snapshot/Merkle-sum worker | unit, fuzz, invariant, shared Solidity/SDK fixture and two-provider SDK tests | holder never claims; keeper pushes and retries; funder or project Treasury can abort a defective epoch after 30 days |
...
| Permanent Liquidity | project-bound `ProjectLiquidityManagerV2` with V3/V4 support and no principal exit | unit, fuzz, Router integration and all-module launch | first funder freezes off-chain-reviewed venue plumbing for its isolated account; creator-facing presets hide those fields |
...
- The staked vote source intentionally suppresses only positions owned by the burn address; unlike
  liquid voting, it does not apply the broader project-custody exclusion set. Governance UIs and
  audit evidence must expose that asymmetry.
...
- Router, Treasury, Basket, Bands, Raffle, Airdrop, and Liquidity use typed project-bound funding or
  control paths and expose no `delegatecall`, proxy upgrades, or generic arbitrary execution. The
  Multisig controller is the intentionally general governance executor and sits outside that custody
  claim.
- Failed delivery remains an exact backed retryable liability. Superseded Router escrow requires
  governance recovery, and a timed Airdrop abort returns only an epoch's unpaid remainder. The
  stateful custody and two system invariant suites exercise cross-module conservation and control.
...
- `forge build --sizes`: pass. Tightest core margins are 1,300 bytes for the deployment engine,
  1,450 bytes for Router, and 1,807 bytes for Launcher.
- `forge test`: 448 passed, 0 failed, 1 environment-gated fork test skipped.
