...
import { buildLiquidityAccountConfig, buildLiquidityLaunchAccountConfig, encodeLiquidityAccountConfig, encodeLiquidityCollectCall, encodeLiquidityFeeDeliveryCall, encodeLiquidityMintCall, encodeLiquidityProtocolFeeDeliveryCall, liquidityFeeDestination, liquidityVenue, projectLiquidityManagerV2Abi, } from "../src/index.js";
...
const approvalProof = [`0x${"11".repeat(32)}`];
test("hydrates exact Liquidity config from product choices and a reviewed profile", () => {
    const config = buildLiquidityAccountConfig({
        ...
        choices: {
            quoteSwapBps: 5_000,
            minNotionalPerMint: 1000n,
            maxNotionalPerMint: 1000000n,
            minMintInterval: 3600n,
    ...
    assert.deepEqual(decodeAbiParameters(fundingConfigTuple, encodeLiquidityAccountConfig(config, approvalProof))[0], { config, integrationApprovalProof: approvalProof });
...
test("encodes the complete permissionless Liquidity work lifecycle", () => {
    const mint = decodeFunctionData({
        ...
        data: encodeLiquidityMintCall({ manager, funder: treasury, subject, notional: 10000n }).data,
    });
    assert.deepEqual(mint, {
        functionName: "mint",
        args: [treasury, subject, 10000n, 0n, "0x"],
    });
