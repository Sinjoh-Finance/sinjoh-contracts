...
const leafParameters = parseAbiParameters("bytes32,uint256,address,bytes32,uint64,uint64,uint48,address,uint256,uint256");
const nodeParameters = parseAbiParameters("bytes32,bytes32,uint256,uint256,uint32,bytes32,uint256,uint256,uint32");
/**
...
export function reconcileAirdropSnapshots(parameters) {
    ...
    if (primary.snapshotBlock !== secondary.snapshotBlock
    ...
    if (primary.totalEligibleWeight !== primaryTotal
...
export function buildAirdropEpoch(parameters) {
    ...
    if (parameters.epochAmount <= 0n)
        throw new RangeError("Epoch amount must be greater than zero");
    ...
    if (parameters.expectedTotalEligibleWeight !== undefined
    ...
    while (level.length > 1) {
        ...
        for (let index = 0; index < level.length; index += 2) {
            ...
            if (left === undefined)
                throw new Error("Invalid Airdrop tree state");
            ...
            for (const member of left.members)
                proofs[member]?.push(proofNode(right, false));
            for (const member of right.members)
                proofs[member]?.push(proofNode(left, true));
            next.push({
...
function canonicalWeights(supplied, suppliedExclusions) {
    if (supplied.length === 0 || BigInt(supplied.length) > MAX_UINT32) {
        throw new RangeError("Airdrop artifact must contain at least one holder");
    }
    const exclusions = new Set((suppliedExclusions ?? []).map((address) => {
    ...
    return supplied.map(({ holder, weight }) => {
        ...
        if (weight <= 0n)
            throw new RangeError("Every Airdrop holder must have positive weight");
        const normalized = holder.toLowerCase();
        if (exclusions.has(normalized))
            throw new RangeError(`Excluded holder ${holder} cannot receive`);
        if (seen.has(normalized))
            throw new RangeError(`Duplicate Airdrop holder ${holder}`);
        seen.add(normalized);
...
export function airdropCommitmentTypedData(chainId, airdrop, commitment) {
    return {
        domain: {
        ...
        primaryType: "AirdropEpochCommitment",
        types: {
...
function airdropNodeHash(left, right) {
...
function proofNode(node, siblingOnLeft) {
    return {
...
function assertUnsigned(value, bits, label) {
    const maximum = bits === 64n ? MAX_UINT64 : bits === 48n ? MAX_UINT48 : 2n ** bits - 1n;
    if (value < 0n || value > maximum)
        throw new RangeError(`${label} exceeds uint${bits}`);
}
