...
export function reconcileRaffleSnapshots(parameters) {
    ...
    if (primaryWeights.length !== secondaryWeights.length
...
export function buildRaffleRound(parameters) {
    ...
    if (parameters.tokensPerTicket <= 0n)
        throw new RangeError("Tokens per ticket must be positive");
    if (parameters.maxTicketsPerHolder < 0n)
        throw new RangeError("Ticket cap cannot be negative");
    ...
    while (level.length > 1) {
        ...
        for (let index = 0; index < level.length; index += 2) {
            ...
            if (left === undefined || right === undefined)
                throw new Error("Invalid Raffle tree state");
    ...
    const root = level[0];
    if (root === undefined || root.sum !== offset)
        throw new Error("Invalid Raffle root sum");
...
function canonicalWeights(weights, exclusions) {
    ...
    return weights.map((entry) => {
        ...
        if (entry.weight < 0n || entry.weight > MAX_UINT256)
            throw new RangeError("Raffle weight must fit uint256");
        const holder = entry.holder.toLowerCase();
        if (excluded.has(holder))
            throw new RangeError(`Excluded holder ${entry.holder} cannot receive tickets`);
        if (seen.has(holder))
            throw new RangeError(`Duplicate Raffle holder ${entry.holder}`);
        seen.add(holder);
