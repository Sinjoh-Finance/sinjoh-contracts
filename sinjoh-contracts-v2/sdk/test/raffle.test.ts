...
import {
  buildRaffleRound,
  buildVerifiedRaffleRound,
  encodeRaffleClaimCalls,
  encodeRaffleCloseCall,
  encodeRaffleCommitCall,
  encodeRaffleRetryCall,
  encodeRaffleClaimOwedToCall,
  projectRaffleV2Abi,
  raffleWinningIndex,
  reconcileRaffleSnapshots,
  reconstructRaffleSnapshot,
} from "../src/index.js";
...
const treeFixture = JSON.parse(await readFile(
  resolve(process.cwd(), "../../sinjoh-raffle-rewards/test/fixtures/ticket-tree.json"),
  "utf8",
)) as TreeFixture;

const slotFixture = JSON.parse(await readFile(
  resolve(process.cwd(), "../../sinjoh-raffle-rewards/test/fixtures/slot-indices.json"),
  "utf8",
)) as {
  chainId: number;
  raffle: Address;
