...
import {
  raffleWinningIndex,
  type BuiltRaffleRound,
  type RaffleSnapshotArtifact,
  type RaffleWeight,
} from "./raffle.js";
...
export function reconstructRaffleSnapshot(parameters: {
  ...
  if (parameters.snapshotBlock < 0n) throw new RangeError("Snapshot block cannot be negative");
  if (parameters.totalSupply < 0n) throw new RangeError("Historical supply cannot be negative");
  ...
  for (const transfer of parameters.transfers) {
    ...
    if (!windowInitialized && transfer.blockNumber >= windowStart) {
      for (const [holder, balance] of balances) minima.set(holder, balance);
      windowInitialized = true;
    }
    const from = transfer.from.toLowerCase();
    const to = transfer.to.toLowerCase();
    if (parameters.basis === "min-balance" && transfer.blockNumber >= windowStart) {
      if (from !== ZERO_ADDRESS && !minima.has(from)) minima.set(from, balances.get(from) ?? 0n);
      if (to !== ZERO_ADDRESS && !minima.has(to)) minima.set(to, balances.get(to) ?? 0n);
    }
    if (from !== ZERO_ADDRESS) debit(balances, from, transfer.value);
    if (to !== ZERO_ADDRESS) balances.set(to, (balances.get(to) ?? 0n) + transfer.value);
    if (parameters.basis === "min-balance" && transfer.blockNumber >= windowStart) {
      if (from !== ZERO_ADDRESS) updateMinimum(minima, from, balances.get(from) ?? 0n);
      if (to !== ZERO_ADDRESS) updateMinimum(minima, to, balances.get(to) ?? 0n);
    }
  }
  ...
  const weights: RaffleWeight[] = [...source.entries()]
...
export function encodeRaffleClaimCalls(parameters: {
  ...
  for (let slot = 0; slot < parameters.round.winnersPerRound; slot += 1) {
    ...
    const leafIndex = parameters.round.leaves.findIndex(
    ...
    if (leaf === undefined || proof === undefined) throw new Error("Winning ticket has no Raffle leaf");
    calls.push({
      kind: "claim",
      to: parameters.round.raffle,
      value: 0n,
...
function validateOrderedTransfers(transfers: readonly HolderTransferEvent[], snapshotBlock: bigint): void {
  let previous: HolderTransferEvent | undefined;
  for (const transfer of transfers) {
    if (transfer.removed) throw new RangeError("Removed transfer logs cannot enter a Raffle snapshot");
    if (
...
function compareEvents(left: HolderTransferEvent, right: HolderTransferEvent): number {
...
function debit(balances: Map<string, bigint>, holder: string, amount: bigint): void {
  const current = balances.get(holder) ?? 0n;
  if (current < amount) throw new RangeError("Raffle transfer history starts late or is inconsistent");
  balances.set(holder, current - amount);
}

function updateMinimum(minima: Map<string, bigint>, holder: string, balance: bigint): void {
  const current = minima.get(holder);
  if (current === undefined || balance < current) minima.set(holder, balance);
}

function exclusionSet(exclusions: readonly Address[]): Set<string> {
  const result = new Set<string>([ZERO_ADDRESS, BURN_ADDRESS]);
