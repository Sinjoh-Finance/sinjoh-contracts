...
export type StakingPositionEvent =
  | (OrderedChainEvent & {
...
export function reconstructHolderAirdropSnapshot(parameters: {
  ...
  for (const transfer of parameters.transfers) {
    ...
    if (from !== ZERO_ADDRESS) debit(balances, from, transfer.value, "Transfer history");
    if (to !== ZERO_ADDRESS) credit(balances, to, transfer.value);
  }
...
export function reconstructStakerAirdropSnapshot(parameters: {
  ...
  const positions = new Map<string, { owner: Address; amount: bigint }>();
  ...
  for (const event of parameters.events) {
    ...
    if (event.eventName === "PositionCreated") {
      ...
      if (positions.has(id)) throw new RangeError(`Duplicate staking position ${id}`);
      positions.set(id, { owner: getAddress(event.owner), amount: event.amount });
      continue;
    }
    const position = positions.get(id);
    if (position === undefined) throw new RangeError(`Unknown staking position ${id}`);
    if (event.eventName === "PositionTransferred") {
      ...
      if (
        position.owner.toLowerCase() !== event.from.toLowerCase()
      ...
      if (
        position.owner.toLowerCase() !== event.owner.toLowerCase()
          || position.amount !== event.amount
...
export function planAirdropPushBatches(parameters: {
  ...
  if (
    !Number.isSafeInteger(parameters.maxPushBatchSize)
  ...
  const batches: AirdropPushBatch[] = [];
  for (let offset = 0; offset < pending.length; offset += parameters.maxPushBatchSize) {
    ...
    batches.push({
      ...
      proofs: entries.map(({ proof }) => {
        if (proof === undefined) throw new RangeError("Missing Airdrop proof");
        return proof;
      }),
    });
  }
...
export function encodeAirdropRetryCreditCall(parameters: {
  ...
  if (parameters.maxAmount <= 0n) throw new RangeError("Retry amount must be greater than zero");
  return {
    kind: "retry-credit",
    to: getAddress(parameters.airdrop),
    value: 0n,
...
function validateOrderedEvents(
  events: readonly OrderedChainEvent[],
  snapshotBlock: bigint,
): void {
  let previous: OrderedChainEvent | undefined;
  for (const event of events) {
    if (event.removed) throw new RangeError("Removed logs cannot be used for an Airdrop snapshot");
    if (
...
function compareEventPosition(left: OrderedChainEvent, right: OrderedChainEvent): number {
...
function exclusionSet(supplied: readonly Address[]): Set<string> {
  const exclusions = new Set<string>([ZERO_ADDRESS, BURN_ADDRESS]);
...
function debit(balances: Map<string, bigint>, holder: string, amount: bigint, label: string): void {
  const current = balances.get(holder) ?? 0n;
  if (current < amount) throw new RangeError(`${label} spends more than the reconstructed balance`);
  const next = current - amount;
  if (next === 0n) balances.delete(holder);
  else balances.set(holder, next);
}

function credit(balances: Map<string, bigint>, holder: string, amount: bigint): void {
...
function assertEligibleOwner(value: Address, label: string): void {
