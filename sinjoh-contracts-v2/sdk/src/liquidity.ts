...
export function buildLiquidityAccountConfig(parameters: {
  ...
  if (profile.id.trim().length === 0) throw new RangeError("Liquidity profile ID cannot be empty");
  if (profile.venue !== 0 && profile.venue !== 1) throw new RangeError("Liquidity venue is unsupported");
  ...
  if (
    !Number.isInteger(profile.maxMintSlippageBps)
...
function feeModeFor(destination: LiquidityFeeDestination): 0 | 1 | 2 | 3 {
  if (destination === "creator") return 0;
  if (destination === "treasury") return 1;
  if (destination === "recycle") return 2;
  if (destination === "funder") return 3;
  throw new RangeError("Liquidity fee destination is unsupported");
}

function keeperCall(kind: LiquidityKeeperCall["kind"], manager: Address, data: Hex): LiquidityKeeperCall {
  return { kind, to: getAddress(manager), value: 0n, data };
}

function assertAddress(value: Address, label: string, allowZero = false): void {
