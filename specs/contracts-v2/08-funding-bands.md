...
Only project governance may create/fund a band. Creation checks the current TWAP-derived platform
...
Creation and funding occur atomically. A pending governance proposal does not reserve inventory or
freeze market-cap eligibility. If market cap reaches/exceeds the lower bound before execution, the
transaction reverts and the proposal can be replaced with a valid band.

## 5. Funding and position custody

The Treasury/governance supplies project-token inventory. The contract measures exact receipt and
mints/increases one one-sided position per band using only canonical pool/ticks derived from the
band configuration.
...
```text
NONE -> ACTIVE -> ARMED -> SETTLED
                    |
                    +-> ACTIVE (price reversal before confirmation)
```

`armSettlement` is permissionless after the live/TWAP price reaches the configured settlement
boundary. A sustained confirmation window and fresh advancing observations prevent single-block
crossing manipulation. The default confirmation period is 15 minutes; factory bounds are 5 minutes
to 24 hours.

`settle` is permissionless after uninterrupted confirmation. It removes 100% of band liquidity,
...
A caller cannot substitute a route or weaken minimum output.

## 8. Failure handling

Settlement state changes to SETTLED only after the position is closed and all proceeds are measured.
...
- `BandDeliveryEscrowed/Delivered/Recovered(...)`.

Views return requested/effective bounds, current platform market cap and source timestamps,
position ID, inventory/proceeds accounting, settlement eligibility, and destination config.
