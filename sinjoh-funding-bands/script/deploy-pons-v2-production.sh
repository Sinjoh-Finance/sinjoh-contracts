...
: "${GUARD_SIGNER:?set the dedicated Funding Bands guard signer}"
: "${ORACLE_SIGNER:?set the dedicated Funding Bands oracle signer}"
...
for address in "$WETH" "$V3_FACTORY" "$V3_POSITION_MANAGER" "$V4_POSITION_MANAGER" \
...
read -r oracle oracle_tx <<< "$(deploy src/oracles/SinjohSignedEthUsdOracle.sol:SinjohSignedEthUsdOracle \
  --constructor-args "$GOVERNANCE" "$ORACLE_SIGNER")"
...
read -r guard guard_tx <<< "$(deploy src/profiles/SinjohV4SignedBandPriceGuard.sol:SinjohV4SignedBandPriceGuard \
  --constructor-args "$V4_STATE_VIEW" "$state_view_hash" "$pool_manager_hash" \
  "$GOVERNANCE" "$GUARD_SIGNER")"
...
[[ "$(lower "$(cast call "$oracle" 'owner()(address)' "${RPC_ARGS[@]}")")" == "$(lower "$GOVERNANCE")" ]]
[[ "$(lower "$(cast call "$oracle" 'quoteSigner()(address)' "${RPC_ARGS[@]}")")" == "$(lower "$ORACLE_SIGNER")" ]]
[[ "$(lower "$(cast call "$guard" 'owner()(address)' "${RPC_ARGS[@]}")")" == "$(lower "$GOVERNANCE")" ]]
[[ "$(lower "$(cast call "$guard" 'quoteSigner()(address)' "${RPC_ARGS[@]}")")" == "$(lower "$GUARD_SIGNER")" ]]
